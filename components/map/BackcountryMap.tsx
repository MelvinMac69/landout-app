...
export function BackcountryMap({
  ...
  // Fixed origin point for DirectTo — captured from GPS position when DirectTo activates.
  // The magenta line is drawn from origin → destination and stays FIXED on the map.
  // Only the GPS position dot moves with subsequent fixes.
  ...
  // Track whether camera move has been called for the current Direct To session — prevents zoom loop
  ...
  useEffect(() => {
    ...
    if (directToDest) {
      ...
      // DEFERRED RENDER: iOS Safari WebGL crash prevention.
      //
      // Every previous approach (fitBounds, flyTo, staggered timing) crashed iOS Safari
      // because camera animations overload the WebGL context during DirectTo activation.
      //
      // Strategy: MINIMAL WebGL on activation.
      // 1. Dest point only at activation (lightest possible GeoJSON)
      // 2. jumpTo for camera (instant snap, zero animation overhead)
      // 3. Route line draws 1 second AFTER camera settles (heaviest operation last)
      //
      // Timeline (from DirectTo activation):
      //   T+1s (cold T+2s): Frame 1 — draw destination point (1 GeoJSON point)
      //   T+1.3s (cold T+2.3s): Frame 2 — jumpTo midpoint (instant, no animation)
      //   T+2.3s (cold T+3.3s): Frame 3 — draw origin + route line (1s after camera)
      //   T+2.6s (cold T+3.6s): Frame 4 — dispatch panel event
      //
      const STAGGER_MS = 300;
      const CAMERA_SETTLE_MS = 1000; // wait for jumpTo to settle before heavy route draw
      const MAX_RETRIES = 5;
      ...
      const deferredRender = (attempt: number) => {
        ...
        directToRenderTimerRef.current = setTimeout(() => {
          ...
          // Frame 2: Camera move using jumpTo (INSTANT snap, NO animation)
          // fitBounds and flyTo both crashed iOS Safari — jumpTo has zero animation overhead.
          setTimeout(() => {
            if (!map.current) return;
            try {
              if (directToOriginCapture && Number.isFinite(directToOriginCapture.lon) && Number.isFinite(directToOriginCapture.lat)
                ...
                map.current.jumpTo({ center: [midLon, midLat], zoom });
              } else if (Number.isFinite(directToDestCapture.lng) && Number.isFinite(directToDestCapture.lat)) {
                map.current.jumpTo({ center: [directToDestCapture.lng, directToDestCapture.lat], zoom: 12 });
              }
            } catch (e) {
              console.warn('[Map] Frame 2 — jumpTo error:', e);
            }

            // Frame 3: Draw origin point + route line — 1 SECOND after jumpTo settles
            // This is the heaviest WebGL op. GPU must be completely idle.
            setTimeout(() => {
              ...
              // Frame 4: Dispatch event for panel UI update
              setTimeout(() => {
                window.dispatchEvent(new CustomEvent('landoutDirectToChange', {
                  detail: { dest: directToDestCapture, currentPos: currentPosState, origin: directToOriginCapture },
                }));
              }, STAGGER_MS);
            }, CAMERA_SETTLE_MS); // 1s after jumpTo settles
          }, STAGGER_MS);
        }, (isFirstDirectToRef.current ? 2000 : 1000) + (attempt * 500)); // Cold start: 2s, subsequent: 1s
      };
    ...
    deferredRender(0);
      ...
      setDirectToOrigin(null);
      directToOriginRef.current = null;
      ...
      requestAnimationFrame(() => {
        window.dispatchEvent(new CustomEvent('landoutDirectToChange', {
          detail: { dest: null, currentPos: null, origin: null },
        }));
      });
  ...
  // Expose DirectTo setter so page.tsx and MapContext can activate/cancel DirectTo.
  // Wrapper captures origin from current GPS position on activation.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    (window as any).__landoutSetDirectToDest = (dest: DirectToDest | null) => {
      if (dest) {
        // Capture origin from current GPS position
        const currentPos = currentPosRef.current;
        const origin = currentPos && (currentPos.lat !== 0 || currentPos.lon !== 0)
          ? { lat: currentPos.lat, lon: currentPos.lon }
          : null;
        setDirectToOrigin(origin);
        directToOriginRef.current = origin;
        // Exit follow mode + set settling guard
      ...
      try {
        ...
        if (directToDestRef.current && map.current && !flyToInProgressRef.current) {
          ...
          if (src) {
            ...
            // Aviation behavior: the line stays FIXED from origin → destination.
            // Only the GPS position dot moves with each fix.
            requestAnimationFrame(() => {
              try {
                ...
                // Fixed line from origin → destination (not tracking device)
                if (origin) {
                  features.push({
                    type: 'Feature',
                    geometry: {
                      type: 'LineString',
                      coordinates: [[origin.lon, origin.lat], [dest.lng, dest.lat]],
                    },
                    properties: {},
                  });
                  // Origin point (fixed)
                ...
                // Destination point (fixed)
                features.push({
                  type: 'Feature',
                  geometry: { type: 'Point', coordinates: [dest.lng, dest.lat] },
                  properties: { role: 'destination' },
                });
                // Current GPS position dot (moves with each fix)
                if (pos) {
                  features.push({
                    type: 'Feature',
                    geometry: { type: 'Point', coordinates: [pos.lon, pos.lat] },
                    properties: { role: 'current-pos' },
                  });
                }
  ...
  useEffect(() => {
    ...
    win.landoutSetDirectTo = (dest) => {
      ...
      // Capture current GPS position as the fixed origin for the DirectTo line.
      const currentPos = currentPosRef.current;
      const origin = currentPos && (currentPos.lat !== 0 || currentPos.lon !== 0)
        ? { lat: currentPos.lat, lon: currentPos.lon }
        : null;
  ...
  function handleDirectTo(lng: number, lat: number, name?: string) {
    ...
    // Capture current GPS position as the fixed origin for the DirectTo line.
    // If no GPS position available, set origin to null — the deferred render
    // will show a "GPS required" warning instead of drawing a line.
    const currentPos = currentPosRef.current;
    const origin = currentPos && (currentPos.lat !== 0 || currentPos.lon !== 0)
      ? { lat: currentPos.lat, lon: currentPos.lon }
      : null;
    ...
    setInfoCard(null);
    console.log('[DirectTo] handleDirectTo:', { lng, lat, name, origin });
    // Guard: suppress rapid WebGL mutations for 1000ms
    directToSettlingRef.current = true;
    setTimeout(() => { directToSettlingRef.current = false; }, 1000);
    setDirectToDest({ lng, lat, name, type: 'map' });
    // Dispatch GPS event synchronously
    try {
      window.dispatchEvent(new CustomEvent('landoutDirectToGps'));
    } catch (e) {
      console.error('[DirectTo] landoutDirectToGps dispatch error:', e);
