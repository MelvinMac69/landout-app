...
export default function SiteDetailFallbackPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto p-4">
        ...
        <Card>
          ...
          <Link href="/sites/search"><Button className="mt-4 w-full">Back to Search</Button></Link>
        </Card>
      </div>
    );
  }
  ...
  if (!site) {
    return (
      <div className="max-w-2xl mx-auto p-4">
        <div className="flex items-center gap-3 mb-4">
          <Link href="/sites/search" className="p-2 -ml-2"><ArrowLeft className="w-5 h-5 text-slate-600" /></Link>
    ...
    <div className="max-w-2xl mx-auto p-4 pb-28">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button
          ...
          className={`p-2 rounded-lg transition-colors flex-shrink-0 ${saved ? 'bg-blue-100 text-blue-600' : 'hover:bg-slate-100 text-slate-400'}`}
        >
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <Card padding="sm" className="text-center">
          ...
          <p className="text-lg font-semibold text-slate-900">{site.runway_length_ft ? `${site.runway_length_ft.toLocaleString()} ft` : '—'}</p>
          <p className="text-xs text-slate-500 capitalize">{site.type_label}</p>
        </Card>
        <Card padding="sm" className="text-center">
          <p className="text-xs text-slate-500">Elevation</p>
          <p className="text-lg font-semibold text-slate-900">{site.elevation_ft ? `${site.elevation_ft.toLocaleString()} ft` : '—'}</p>
        </Card>
        <Card padding="sm" className="text-center">
          <p className="text-xs text-slate-500">Type</p>
          <p className="text-sm font-semibold capitalize" style={{ color: typeColor(site.type_label) }}>{site.type_label}</p>
        </Card>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-6 flex items-start gap-2">
        ...
        <p className="text-xs text-amber-800">Land status shown on map. This does <strong>not</strong> authorize landing. Always verify access permissions.</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 mb-4 bg-slate-100 rounded-lg p-1">
        <button onClick={() => setActiveTab('reports')} className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'reports' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
          Reports ({mockReports.length})
        </button>
        <button onClick={() => setActiveTab('info')} className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'info' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
          Info
        </button>
      </div>
      ...
      {activeTab === 'reports' ? (
        <div className="space-y-4">
          <button onClick={() => alert('Condition report form coming soon!')} className="w-full">
            <Button variant="outline" className="w-full gap-2"><Clock className="w-4 h-4" />Add Condition Report</Button>
          </button>
          {mockReports.map((report) => (
            <Card key={report.id}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${report.report_type === 'hazard' ? 'bg-red-100 text-red-700' : report.report_type === 'closure' ? 'bg-slate-100 text-slate-700' : 'bg-green-100 text-green-700'}`}>
                    {report.report_type}
                  </span>
              ...
              <div className="flex items-center gap-1 mt-2 text-xs text-slate-500"><User className="w-3 h-3" />{report.reported_by}</div>
            </Card>
          ))}
        </div>
      ) : (
      ...
      <div className="fixed bottom-20 md:bottom-4 left-4 right-4 max-w-2xl mx-auto">
        <div className="flex gap-2">
          <Button variant="secondary" className="flex-1 gap-2" onClick={() => navigator.clipboard.writeText(window.location.href)}>
